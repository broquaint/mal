...
fun eval_ast(ast: MalType, env: Env) =
    when(ast) {
    ...
    eval_loop@ while (true) {
        ...
        if (ast !is MalList) {
            ...
            if (ast.atoms.isEmpty()) {
                ...
                if(next is MalSymbol) {
                    when(next.sym) {
                        ...
                        "let*" -> {
                            ...
                            env = make_env((rest[0] as MalSeq), env)
                            //  Set ast (i.e. the local variable passed in as first parameter of EVAL) to be the second ast argument.
                            ast = ast[2]
                            continue@eval_loop // TCO
                        }
                        "do" -> {
                            // change the eval_ast call to evaluate all the parameters except for
                            // the last (2nd list element up to but not including last).
                            eval_ast(rest.butlast(), env)
                            // Set ast to the last element of ast. Continue at the beginning of the
                            // loop (env stays unchanged).
                            ast = rest.last()
                            continue@eval_loop // TCO
                        }
                        "if" -> {
                            // the condition continues to be evaluated, however, rather than
                            // evaluating the true or false branch, ast is set to the unevaluated
                            // value of the chosen branch.
                            ast = if(is_true(EVAL(rest[0], env))) rest[1] else
                ...
                val op   = eval_ast(ast, env) as MalList
                val func = op.head()
                val args = op.tail()
